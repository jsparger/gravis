(function () {
  ...
  class Vis {
    constructor(graph, width, height) {
      ...
      this._sim = simulation()
        // .velocityDecay(0.1)
        .force("charge", manyBody().strength(-30))
        .force("link", link().distance(50).strength(1))
        .force("center", center$1(this._width/2, this._height/2))
        .on("tick", this.tick.bind(this));
      this._events = ["click", "dblclick", "mousedown", "mouseenter", "mouseleave", "mousemove", "mouseover", "mouseout", "mouseup", "keydown"];
      this.dispatch = dispatch(...this._events);
      this._init();
    }

    _init() {
      // create svg
      this._svg = select("#force").append("svg")
        .attr("id", "forcesvg")
      ...
      this._make_interactive(select("body"));
    ...
    _make_interactive(s) {
      let dispatch$$1 = this.dispatch;
      this._events.map((event$$1) => {
        s.on(event$$1, function (d) {event.stopPropagation(); dispatch$$1.call(event$$1, this, d);} );
      });
    }
  ...
  class Interact {
    constructor(vis) {
      ...
      this.dispatch = dispatch(...this.events);
      this.selected = null;
  ...
  function colorize(element) {
    if (!element) return;
    let s = select(element);
    let d = s.data()[0];
    let color$$1 = choose_color(JSON.parse(s.attr("status")));
    if (is_node(d)) {
      s.style("fill", color$$1);
    }
    else if (is_relationship(d)) {
      s.selectAll("line").style("stroke", color$$1);
    }
  }
  ...
  function find_selected(svg$$1) {
    let selected = null;
    selectAll("[status]").each( function (d) {
      if ("select" in JSON.parse(select(this).attr("status"))) {
        selected = this;
      }
    });
    return selected;
  ...
  class Actions {
    ...
    highlight_selected_entity() {
      let dispatch$$1 = this._int.dispatch;
      let name = "highlight_selected_node";
      let selected = null;

      dispatch$$1.on(`select.${name}`, function (d) {
        dispatch$$1.call("deselect");
        add_status(this, "select");
        colorize(this);
        selected = this;
      });

      dispatch$$1.on(`deselect.${name}`, function () {
    ...
    highlight_hover_entity() {
      let dispatch$$1 = this._int._vis.dispatch;
      let name = "highlight_hover_entity";

      dispatch$$1.on(`mouseenter.${name}`, function (d) {
        dispatch$$1.call("mouseleave");
      ...
      dispatch$$1.on(`mouseleave.${name}`, function (d) {
    ...
    create_node_on_shift_click() {
      ...
      dispatch$$1.on(`create.${name}`, function (d) {
        let c = mouse(this);
        let x = null;
        let sd = select(find_selected()).data()[0];
    ...
    delete_selected_node() {
      ...
      dispatch$$1.on(`delete.${name}`, function (d) {
        let sd = select(find_selected()).data()[0];
  ...
  let c$3 = {id: 2, name: "C"};
  let d = {id: 3, name: "D"};
  let r = {id: 0, source: a$1, target: b, name: "r"};
  let r2 = {id:1, source: a$1, target: c$3, name: "r2"};

  let w = 500; let h = 250;
  let graph = new Graph();
  let vis = new Vis(graph, w, h);
  vis.dispatch.on("click.visualize", function (d) {console.log("Got click event in visualization.html", this, d);});
  let int = new Interact(vis);
  ...
  setTimeout(() => {
    graph.add(r2);
    graph.add(d);
    graph.add({id: 2, source: d, target: c$3});
    vis.update();
  }, 3000);
